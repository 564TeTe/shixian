...
export default {
  ...
  watch: { '$route.query.termId'() { this.result = null } },
  mounted() {
    this.loadStatus()
  },
  methods: {
    ...
    async query() {
      ...
      this.saving = true
      this.result = null
      const queryTerm = this.$route.query.termId || ''
      try {
        const result = await request('/ai/query', {
          method: 'post',
          data: { question: this.question, termId: this.$route.query.termId || '' },
          timeout: 120000
        })
        if ((this.$route.query.termId || '') === queryTerm) this.result = result
      } catch (e) {
        this.fail(e)
      } finally {
        this.saving = false
