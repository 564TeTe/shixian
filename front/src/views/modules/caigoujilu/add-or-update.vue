<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="设备编号" prop="shebeibianhao">
          <el-select  @change="shebeibianhaoChange" v-model="ruleForm.shebeibianhao" placeholder="请选择设备编号">
            <el-option
                v-for="(item,index) in shebeibianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.shebeibianhao" label="设备编号" prop="shebeibianhao">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="采购数量" prop="caigoushuliang">
          <el-input v-model="ruleForm.caigoushuliang" 
              placeholder="采购数量" clearable  :readonly="ro.caigoushuliang"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="采购数量" prop="caigoushuliang">
              <el-input v-model="ruleForm.caigoushuliang" 
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="采购价格" prop="caigoujiage">
          <el-input v-model="ruleForm.caigoujiage" 
              placeholder="采购价格" clearable  :readonly="ro.caigoujiage"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="采购价格" prop="caigoujiage">
              <el-input v-model="ruleForm.caigoujiage" 
                placeholder="采购价格" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="采购公司" prop="caigougongsi">
          <el-input v-model="ruleForm.caigougongsi" 
              placeholder="采购公司" clearable  :readonly="ro.caigougongsi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="采购公司" prop="caigougongsi">
              <el-input v-model="ruleForm.caigougongsi" 
                placeholder="采购公司" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="采购日期" prop="caigouriqi">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.caigouriqi" label="采购日期" prop="caigouriqi">
              <el-input v-model="ruleForm.caigouriqi" 
                placeholder="采购日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  data() {
	...
	beizhu : false,
      ...
      rules: {
          ...
          caigougongsi: [
                { required: true, message: '采购公司不能为空', trigger: 'blur' },
          ],
          caigouriqi: [
          ],
          beizhu: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.caigoujiage = true;
          ...
          if(o=='caigougongsi'){
            this.ruleForm.caigougongsi = obj[o];
	    this.ro.caigougongsi = true;
            continue;
          }
          if(o=='caigouriqi'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/shiyanshebei/shebeibianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.shebeibianhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    shebeibianhaoChange () {
      this.$http({
        url: `follow/shiyanshebei/shebeibianhao?columnValue=`+ this.ruleForm.shebeibianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.shebeimingcheng){
                this.ruleForm.shebeimingcheng = data.data.shebeimingcheng
              }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `caigoujilu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.caigoujiluCrossAddOrUpdateFlag = false;
                  this.parent.search();
