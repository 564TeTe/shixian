...
本项目不部署模型推理服务，也不要求每台电脑安装本地模型。所有用户通过系统后端调用同一个云端 OpenAI 兼容接口：

```text
浏览器用户 A ─┐
浏览器用户 B ─┼─> Spring Boot 应用 ──共享云端模型 API
浏览器用户 C ─┘          │
                         └── MySQL 只读查询账号
```

云端模型 API Key 只配置在后端运行环境中，浏览器永远不接触 Key。共享的是模型服务和配额，不共享用户会话、查询历史、权限和数据库结果。

### 13.2 两种密钥模式

#### 模式 A：平台统一密钥（推荐）

管理员申请一个云模型平台账号，在后端配置：

```text
TEACHING_AI_BASE_URL=https://api.example.com/v1
TEACHING_AI_API_KEY=平台统一密钥
TEACHING_AI_MODEL=模型名称
```

所有用户的请求都由后端使用该 Key 发出。平台账单、限流和审计集中管理，用户无需配置任何密钥。为避免用户之间相互影响，后端必须为每个请求附加 `userId`、`requestId` 和权限上下文，并在应用侧实施限流与配额。

#### 模式 B：用户自带密钥（接受密钥风险时可选）

管理员或用户在个人设置中填写自己的模型 Key。后端收到后只在内存中使用，不写入数据库、不写日志、不返回前端；也可以直接由浏览器调用模型平台，但这种方式会暴露 Key，且无法可靠控制请求内容和费用。

如果确实需要用户自行承担密钥风险，建议仍采用“前端提交 Key → 后端代理调用”的方式：

1. HTTPS 提交 Key，后端校验格式和模型地址白名单。
2. Key 仅绑定当前用户会话，设置短时 TTL（例如 30 分钟）。
3. 内存缓存按 `userId` 隔离，禁止使用全局静态变量。
4. 日志、异常、监控和前端响应全部脱敏。
5. 用户退出登录、修改 Key 或 TTL 到期时立即清除。
6. 提示用户：应用服务器管理员、运行时内存转储和云模型服务商理论上都可能接触该 Key。

### 13.3 用户隔离边界

共享模型本身不会自动隔离用户。隔离必须由应用实现：

| 隔离对象 | 实现方式 |
| --- | --- |
| 登录身份 | 现有登录 Token 解析为 `userId` 和角色 |
| 权限 | Controller/Service 强制校验管理员权限，不能只隐藏前端菜单 |
| 会话历史 | 数据表或缓存键使用 `ai:conversation:{userId}:{conversationId}` |
| 查询结果 | 响应只返回当前请求结果，不使用全局结果变量 |
| 数据范围 | 管理员可查白名单数据；教师必须追加本人课程/任务条件 |
| 速率限制 | 使用 `ai:rate:{userId}`，同时设置全局平台限流 |
| 审计 | 记录 `userId`、requestId、模型、耗时、行数和 SQL 哈希，不记录密钥 |

对于当前需求，建议第一版只开放管理员自然语言查询。若开放教师查询，应为教师建立只读视图，例如 `teacher_visible_task`，让 SQL Guard 只允许访问该视图，避免依赖模型自行添加教师条件。

### 13.4 后端代理接口

继续使用现有接口即可，前端不需要知道模型平台：
...
1. 从登录 Token 获取当前用户。
2. 检查角色、请求频率和问题长度。
3. 读取当前用户允许看到的 Schema。
4. 使用平台统一 Key 或该用户临时 Key 调用云模型。
5. 对模型返回值执行 `TeachingAiSqlGuard`。
6. 使用数据库只读账号执行 SQL。
7. 将结果绑定到当前 requestId 后返回。

不要让前端直接提交任意 `baseUrl`、`model` 或 `sql`。如果支持用户选择模型，只能从后端维护的模型白名单中选择。

### 13.5 共享模式的费用与可用性控制

- 每个用户限制每分钟请求次数和每日 Token 预算。
- 对问题长度、Schema 长度和 `max_tokens` 设置上限。
- 模型请求设置连接超时、读取超时和有限重试；禁止无限重试。
- 云平台返回 429 时提示“当前共享额度已用尽”，不暴露平台密钥或原始错误。
- 记录每次调用的 Token 用量（如果平台提供），按用户统计成本。
- 为模型不可用保留普通报表查询入口，AI 故障不能阻塞教学管理功能。

### 13.6 适用结论

在“无需服务器部署模型、无需本地部署、多人共享且互相隔离”的条件下，采用**平台统一 Key + 后端代理 + 应用层用户隔离 + 数据库只读账号**最合适。该方案只需要购买或申请一个云端模型服务，其他用户通过系统账号直接使用；密钥风险集中在后端运行环境和云模型供应商，浏览器端不暴露密钥。
