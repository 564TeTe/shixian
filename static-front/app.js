...
(() => {
  ...
  function overview() {
    ...
    return heading('TEACHING WORKSPACE', `${admin() ? '管理员' : esc(teacherName())}，欢迎回来`, '把教学安排得井井有条，让每一次实验都有所收获。', `${button('查看本周课表', 'open-week', 'calendar')}${canManageProjects() ? button('新增实验项目', 'new-project', 'plus', 'primary') : ''}`) +
      `<section class="semester-banner"><di+ v><span class="banner-label"><span class="status-dot"></span>${current() ? '新学期 · 教学进行中' : '历史学期 · 已归档'}</span><h2>${termName()}</h2><p>从课程计划到实验项目，在这里连接教学的每一步。</p><a href="#projects">进入实验项目 ${icon('arrow')}</a></div><div class="banner-art" aria-hidden="true"><div class="orbit orbit-one"></div><div class="orbit orbit-two"></div><div class="art-card art-back">${icon('book')}<i></i><i></i></div><div class="art-card art-front">${icon('flask')}<span>LAB</span><div class="art-lines"><i></i><i></i></div></div><span class="art-spark">✦</span><span class="art-dot"></span></div><div class="banner-week"><strong>${current() ? '02' : '16'}</strong><span>教学周 / 16 周</span><div class="mini-progress"><i style="width:${current() ? 12.5 : 100}%"></i></div></div></section>` +
      stats([
