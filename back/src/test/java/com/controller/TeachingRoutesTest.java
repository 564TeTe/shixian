...
class TeachingRoutesTest {
    @Test
    void authorizationUsesResolvedRouteInsteadOfRawPrefix() throws Exception {
        for (String raw : new String[] {"/teaching/../users/list", "/teaching/%2e%2e/users/list"}) {
            ...
            HandlerMethod handler =
                    new HandlerMethod(
                            ...
                            java.util.Arrays.stream(AccountController.class.getDeclaredMethods())
                                    .filter(m -> m.getName().equals("loginAdministrator"))
                                    .findFirst()
                                    .get());
            assertFalse(new AuthorizationInterceptor(null).preHandle(request, response, handler));
            assertTrue(response.getContentAsString().contains("404"));
        }
    }

    @Test
    void oldAnonymousResetAndGenericDatabaseRoutesAreClosed() {
        for (String path :
                new String[] {
                    "/users/resetPass",
                    "/jiaoshi/resetPass",
                    "/users/register",
                    "/shiyankecheng/detail/1",
                    "/option/jiaoshi/gonghao",
                    "/option/teacher/teacher_no",
                    "/users/update",
                    "/jiaoshi/info/1"
                }) {
